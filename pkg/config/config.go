...
// env vars should start with AGENTKUBE_CONFIG_ and use _ as separator
// If a value is set both in flags and env then flag takes priority.
// eg:
// export AGENTKUBE_CONFIG_PORT=2344
...
func Parse(args []string) (*Config, error) {
	...
	if err := k.Load(env.Provider("AGENTKUBE_CONFIG_", ".", func(s string) string {
		return strings.ReplaceAll(strings.ToLower(strings.TrimPrefix(s, "AGENTKUBE_CONFIG_")), "_", "-")
	}), nil); err != nil {
...
func flagset() *flag.FlagSet {
	...
	f.String("base-url", "", "Base URL path. eg. /operator")
