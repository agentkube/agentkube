...
const AddResourceLogsPicker: React.FC<AddResourceLogsPickerProps> = ({ onLogsSelect }) => {
  ...
  return (
    <div ref={dropdownRef} className="relative">
      ...
      {isOpen && (
        <div className="absolute left-0 bottom-full mb-1 w-[25rem] rounded-md shadow-lg bg-popover/95 backdrop-blur-sm border border-border z-50">
          <div className="p-2">
            <div className="relative">
              <input
                ...
                className="w-full pl-3 pr-3 py-2 bg-muted rounded text-xs text-foreground focus:outline-none"
                autoFocus
              />
            </div>
          </div>
          ...
          <div className="max-h-48 overflow-y-auto py-1
            ...
            {isLoading && (
              <div className="px-3 py-2 text-xs text-muted-foreground flex items-center justify-center">
            ...
            {!isLoading && !error && pods.map((pod, podIndex) => (
              <div key={`${pod.namespace}-${pod.name}`} className="border-b border-border last:border-b-0">
                {/* Pod Header */}
                <div
                  className="px-2 py-1.5 text-xs cursor-pointer flex items-center justify-between hover:bg-accent-hover text-foreground"
                ...
                {pod.expanded && (
                  <div className="bg-muted/30">
                    {pod.containers.map((container) => (
                      <div
                        key={container}
                        className="px-4 py-1.5 text-xs cursor-pointer flex items-center justify-between hover:bg-accent-hover text-foreground border-l-2 border-border ml-3"
