...
import { Check, ClipboardCopy, CirclePlay, Pencil, Maximize2, AlertCircle } from 'lucide-react';
...
export const CodeBlock = ({
  ...
  return (
    <div 
      ref={containerRef}
      className={`relative my-4 rounded-xl bg-gray-900 text-gray-100 ${isFocused ? 'ring-2 ring-gray-800' : ''}`} 
      tabIndex={0}
      onFocus={handleFocus}
      onBlur={handleBlur}
    ...
    >
        ...
        <div className="flex">
          {showExecuteButton && (
            <button
              ...
              className="px-2 py-1 flex items-center justify-center rounded-[0.3rem] bg-gray-800 hover:bg-gray-700 text-gray-300 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              title={!currentContext ? 'Please select a cluster first' : 'Run (Cmd+Enter)'}
            >
              <CirclePlay className="mr-2" size={14} />
              {isExecuting ? "Running..." : "Run (⌘ + Enter)"}
            </button>
          )}
          <button
            onClick={handleEdit}
      ...
      {result && (
        <>
          {!showOutput ? (
            <div className="px-4 py-2 border-t border-gray-700 bg-gray-800 rounded-b-xl">
              <div className="flex justify-between items-center">
                <div className="flex items-center">
                  <span className="text-sm text-gray-400 mr-2">Command {result.success ? 'succeeded' : 'failed'}</span>
                  {result.success ? (
                    <Check size={14} className="text-green-500" />
                  ) : (
                    <AlertCircle size={14} className="text-red-500" />
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex items-center px-2 py-1 text-xs rounded-[0.3rem] bg-gray-700 hover:bg-gray-600 text-gray-300"
                  >
                    Show Output
                  </button>
                  <button
            ...
            <div className="p-4 border-t border-gray-700 bg-gray-800/50 rounded-b-xl w-full overflow-x-auto">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-2">
                ...
                <button
                  ...
                  className="flex items-center p-2 text-xs rounded-[0.3rem] bg-gray-700 hover:bg-gray-600 text-gray-300"
                >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] bg-gray-200 dark:bg-gray-900">
          ...
          ">
            <div className="px-4 py-2 text-xs text-gray-400 bg-gray-800/50 rounded-t-[0.5rem] w-full overflow-x-auto">
              {language}
            </div>
            <SyntaxHighlighter
              language="bash"
