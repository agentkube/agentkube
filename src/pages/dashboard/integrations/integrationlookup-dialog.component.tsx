...
interface IntegrationLookupDialogProps {
  isOpen: boolean;
  onClose: () => void;
  clusterName: string;
  toolName: string; // e.g., 'argocd', 'vault', etc.
  toolDisplayName: string; // e.g., 'ArgoCD', 'Vault', etc.
  toolIcon?: React.ReactElement;
}

const IntegrationLookupDialog: React.FC<IntegrationLookupDialogProps> = ({
  isOpen,
  onClose,
  clusterName,
  toolName,
  toolDisplayName,
  toolIcon = <Server className="h-4 w-4" />
}) => {
  ...
  useEffect(() => {
    if (isOpen && clusterName && toolName) {
      fetchToolInstances();
    }
  }, [isOpen, clusterName, toolName]);
    ...
    if (toolInstances.length === 1) {
      ...
      if (instance.ports.length === 1) {
        ...
        const webPort = instance.ports.find(p =>
          ...
          [80, 443, 3000, 8080, 8443, 9090].includes(p.port)
        );
        if (webPort) {
  ...
  const fetchToolInstances = async () => {
    try {
      ...
      const response: ToolLookupResponse = await findToolInCluster(clusterName, toolName);
      ...
      if (response.instances && response.instances.length > 0) {
        ...
        toast({
          ...
          description: `No ${toolDisplayName} instances found in cluster ${clusterName}`,
          variant: "destructive"
        });
      }
    } catch (error) {
      ...
      toast({
        title: "Error",
        description: `Failed to find ${toolDisplayName} instances`,
        variant: "destructive"
      });
    } finally {
  ...
  const handlePortForward = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `${toolDisplayName} dashboard available on localhost:${result.port}`,
      });
    } catch (error) {
      toast({
        title: "Error",
      ...
      <DialogContent className="sm:max-w-[500px] bg-gray-200 dark:bg-card backdrop-blur-sm">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            {toolIcon}
            Open {toolDisplayName} Dashboard
          </DialogTitle>
          <DialogDescription>
            Connect to {toolDisplayName} dashboard in cluster {clusterName}
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="grid gap-4 py-4">
            ...
            {fetchingInstances && (
              <div className="flex items-center justify-center py-4">
                ...
                Finding {toolDisplayName} instances...
              </div>
            )}
          </div>
        ) : (
          <div className="py-2 space-y-4">
            <div className="rounded-md bg-gray-50 dark:bg-gray-500/10 p-4 border border-gray-200 dark:border-gray-800">
              <h4 className="font-medium mb-2 flex items-center gap-2">
                {toolIcon}
                {toolDisplayName} Dashboard Active
              </h4>
...
export default IntegrationLookupDialog;
