...
import { ChevronRight, FileText, Clock, CheckCircle, XCircle, AlertCircle, Search, ChevronLeft, ChevronsLeft, ChevronsRight, MoreVertical, TrendingUp, Trash2, RotateCcw, ArrowUpDown, ArrowUp, ArrowDown, RefreshCw, Play, Plus, StopCircle, CheckCircle2 } from 'lucide-react';
...
import { listTasks, getTaskDetails, deleteTask, cancelInvestigation, deleteInvestigation, patchTask } from '@/api/task';
...
const Investigations: React.FC = () => {
  ...
  const toggleTaskResolved = async (task: Task) => {
    try {
      const newResolvedStatus = task.resolved === "yes" ? "no" : "yes"; // Toggle
      await patchTask(task.task_id, newResolvedStatus);

      // Update local state without full refresh for better performance
      setTasks(prevTasks => prevTasks.map(t =>
        t.task_id === task.task_id
          ? { ...t, resolved: newResolvedStatus }
          : t
      ));

      toast(newResolvedStatus === "yes" ? "Task marked as resolved" : "Task marked as unresolved");
    } catch (err) {
      console.error('Error updating task resolution:', err);
                              ...
                              >
                                <TableCell className="font-medium">
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                      <div className="hover:text-blue-500 hover:underline font-medium text-xs flex items-center gap-1">
                                        {task.title}
                                        {task.resolved === "yes" && (
                                          <CheckCircle2 className="w-3 h-3 text-green-500 fill-green-100 dark:fill-green-900/20" />
                                        )}
                                      </div>
                                  ...
                                  <DropdownMenu>
                                    ...
                                    <DropdownMenuContent align="end" className='dark:bg-card/40 backdrop-blur-md border-gray-800/50'>
                                      ...
                                      <DropdownMenuItem
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          toggleTaskResolved(task);
                                        }}
                                        className='hover:text-gray-700 dark:hover:text-gray-500'
                                      >
                                        <CheckCircle2 className={`mr-2 h-4 w-4 ${task.resolved === "yes" ? 'text-green-500' : ''}`} />
                                        {task.resolved === "yes" ? 'Mark as Unresolved' : 'Mark as Resolved'}
                                      </DropdownMenuItem>
