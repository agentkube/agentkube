...
import { SiGrafana, SiDatadog, SiNewrelic, SiPrometheus, SiNodedotjs, SiKubernetes, SiGooglecloud } from '@icons-pack/react-simple-icons';
import { SigNoz } from '@/assets/icons';
import { Check, ArrowUpRight, TriangleAlert, TrendingUp, Server, Network, Settings, ListTree, Search } from 'lucide-react';
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
const Drilldown = () => {
  ...
  const [isMetricsDrawerOpen, setIsMetricsDrawerOpen] = useState<boolean>(false);
  ...
  const renderMetricsDrawerContent = () => {
    const tabs = ['Overview', 'Telemetry', 'Requests', 'Errors', 'Duration'];

    return (
      <>
        <DrawerHeader onClose={() => setIsMetricsDrawerOpen(false)}>
          <div className="py-1">
            <div className='text-lg flex items-center gap-1'>
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                {MOCK_SERVICE_DATA.serviceName}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-sm text-gray-500">
                {MOCK_SERVICE_DATA.namespace}
              </span>
            </div>
          </div>
        </DrawerHeader>
      ...
      <div className="p-6 mx-auto space-y-6">
        ...
        <div className="grid grid-cols-4 grid-rows-6 gap-2 h-[750px]">
          ...
          <div className="dark:bg-gray-800/20 rounded-lg p-4 col-span-1 row-span-3 flex flex-col">
            ...
            {/* View Metrics Button */}
            <Button className='flex justify-between' onClick={handleMetricsClick}>
              Drilldown Metrics
              <ArrowUpRight />
            </Button>
          </div>

          <SideDrawer isOpen={isMetricsDrawerOpen} onClose={() => setIsMetricsDrawerOpen(false)}>
            {renderMetricsDrawerContent()}
          </SideDrawer>
