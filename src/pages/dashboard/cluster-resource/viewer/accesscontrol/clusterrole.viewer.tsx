...
const ClusterRoleViewer: React.FC = () => {
  ...
  const confirmResourceDeletion = async () => {
    if (!clusterRoleData || !currentContext) {
    ...
    try {
      ...
      await deleteResource(
        currentContext.name,
        'clusterroles',
        clusterRoleData.metadata?.name as string,
        {
          // Note: ClusterRoles are cluster-scoped, so no namespace parameter needed
          apiGroup: 'rbac.authorization.k8s.io'
        }
      );

      // Navigate back to the cluster roles list
      navigate('/dashboard/explore/clusterroles');
    } catch (err) {
      console.error('Failed to delete cluster role:', err);
      setError(err instanceof Error ? err.message : 'Failed to delete cluster role');
    } finally {
      ...
      <div className={`p-6 ${fullWidth ? 'max-w-full' : 'max-w-7xl'} mx-auto`}>
        ...
        {clusterRoleData && (
          <DeletionDialog
            ...
            description={`Are you sure you want to delete the cluster role "${clusterRoleData.metadata.name}"? This action cannot be undone.`}
            resourceName={clusterRoleData.metadata.name as string}
