...
const DeploymentPods: React.FC<DeploymentPodsProps> = ({
  ...
  const fetchDeploymentPods = async () => {
    if (!deploymentName || !namespace || !clusterName) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch pods for this deployment');
    } finally {
      setLoading(false);
    }
  };
  ...
  }, [deployment, deploymentName, namespace, clusterName]);
  ...
  // Get container status emoji and hover message
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          Unable to fetch pods for this deployment.
        </div>
      </div>
    );
  }
  ...
  if (pods.length === 0) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-transparent p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          No pods found for this deployment. The deployment may not have created any pods yet.
        </div>
      </div>
    );
  }
                ...
                >
                  ...
                  <TableCell className="text-center">
                    <div title={containerStatus.message} className="flex items-center justify-center gap-1">
                      <span>{containerStatus.icon}</span>
