...
import { V1Pod, V1ReplicationController } from '@kubernetes/client-node';
import { listResources } from '@/api/internal/resources';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Loader2, RefreshCw, MoreVertical, Search, CircleCheck, CircleX, CircleAlert, CircleDashed } from "lucide-react";
...
interface ReplicationControllerPodsProps {
  rcName: string;
  namespace: string;
  clusterName: string;
...
const ReplicationControllerPods: React.FC<ReplicationControllerPodsProps> = ({
  rcName,
  namespace,
  clusterName,
  ...
  // Build label selector from replicationController
  // Note: ReplicationController uses simple equality-based selector instead of matchLabels
  const getLabelSelector = () => {
    if (!replicationController || !replicationController.spec?.selector) {
      return '';
    }

    // Convert the selector to a comma-separated string of key=value pairs
  ...
  const fetchReplicationControllerPods = async () => {
    if (!rcName || !namespace || !clusterName) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Get the replicationController's label selector
      const labelSelector = getLabelSelector();

      if (!labelSelector) {
        throw new Error('Unable to determine label selector for this ReplicationController');
      }

      // Fetch pods matching the replicationController's selector
      ...
      console.error('Failed to fetch pods for ReplicationController:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch pods for this ReplicationController');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Fetch pods when replicationController changes
  useEffect(() => {
    if (replicationController) {
      fetchReplicationControllerPods();
    }
  }, [replicationController, rcName, namespace, clusterName]);
  ...
  const renderResourceTooltip = (
    ...
    return createPortal(
      <div
        className="fixed z-50 bg-white dark:bg-gray-800/90 backdrop-blur-sm min-w-[150px] p-3 rounded-md shadow-lg border border-gray-300 dark:border-gray-700 text-xs"
        style={{
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium">ReplicationController Pods</h2>
          <Button
            variant="outline"
            size="sm"
            onClick={fetchReplicationControllerPods}
            disabled={loading}
          >
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          Unable to fetch pods for this ReplicationController.
        </div>
      </div>
    );
  }
  ...
  if (pods.length === 0) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          No pods found for this ReplicationController. The ReplicationController may not have created any pods yet.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
        <h2 className="text-lg font-medium">ReplicationController Pods ({filteredPods.length})</h2>
        <Button
          variant="outline"
          size="sm"
          onClick={fetchReplicationControllerPods}
...
export default ReplicationControllerPods;