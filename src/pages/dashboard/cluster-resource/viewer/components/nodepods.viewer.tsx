...
const NodePods: React.FC<NodePodsProps> = ({ nodeName, clusterName }) => {
  ...
  const fetchNodePods = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch pods for this node');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchPodsMetrics = async () => {
    if (!clusterName || pods.length === 0) {
      return;
    }

    try {
      const metricsApiUrl = `${OPERATOR_URL}/clusters/${clusterName}/apis/metrics.k8s.io/v1beta1/pods`;
  ...
  const filteredPods = useMemo(() => {
    ...
    return pods.filter(pod => {
      ...
      // Check if name, namespace, status, or IP contains the query
      if (
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          No pods are currently running on this node.
        </div>
      </div>
    );
  }
                ...
                >
                  ...
                  <TableCell>
                    <div className="hover:text-blue-500 hover:underline" onClick={(e) => {
                      e.stopPropagation();
                      navigate(`/dashboard/explore/namespaces/${pod.metadata?.namespace}`);
                    }}>
