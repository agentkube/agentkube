...
interface StatefulSetPodsProps {
...
const StatefulSetPods: React.FC<StatefulSetPodsProps> = ({ 
  statefulSetName, 
  namespace, 
  clusterName,
  statefulSet
  ...
  // Build label selector from statefulSet
  const getLabelSelector = () => {
    if (!statefulSet || !statefulSet.spec?.selector?.matchLabels) {
      return '';
    }

    const labels = statefulSet.spec.selector.matchLabels;
  ...
  const fetchStatefulSetPods = async () => {
    if (!statefulSetName || !namespace || !clusterName) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Failed to fetch pods for this StatefulSet');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Fetch pods when statefulSet changes
  useEffect(() => {
    if (statefulSet) {
      fetchStatefulSetPods();
    }
  }, [statefulSet, statefulSetName, namespace, clusterName]);
  ...
  const getPodOrdinal = (podName: string | undefined): string => {
    if (!podName) return '';
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          Unable to fetch pods for this StatefulSet.
        </div>
      </div>
    );
  }
  ...
  if (pods.length === 0) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          No pods found for this StatefulSet. The StatefulSet may not have created any pods yet.
        </div>
      </div>
    );
  }
        ...
        <Table>
          <TableHeader>
            <TableRow className="border-b border-gray-400 dark:border-gray-800/80">
              ...
              <TableHead className="text-center">Ordinal</TableHead>
...
export default StatefulSetPods;