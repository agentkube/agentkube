...
const ReplicaSetPods: React.FC<ReplicaSetPodsProps> = ({
  replicaSetName,
  namespace,
  clusterName,
  replicaSet
  ...
  // Build label selector from replicaSet
  const getLabelSelector = () => {
    if (!replicaSet || !replicaSet.spec?.selector?.matchLabels) {
      return '';
    }

    const labels = replicaSet.spec.selector.matchLabels;
    return Object.entries(labels)
      .map(([key, value]) => `${key}=${value}`)
      .join(',');
  };

  const fetchReplicaSetPods = async () => {
    if (!replicaSetName || !namespace || !clusterName) {
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      // Get the replicaSet's label selector
      const labelSelector = getLabelSelector();

      if (!labelSelector) {
        throw new Error('Unable to determine label selector for this ReplicaSet');
      }

      // Fetch pods matching the replicaSet's selector
      ...
      console.error('Failed to fetch pods for ReplicaSet:', err);
      setError(err instanceof Error ? err.message : 'Failed to fetch pods for this ReplicaSet');
    } finally {
      setLoading(false);
    }
  };
  ...
  // Fetch pods when replicaSet changes
  useEffect(() => {
    if (replicaSet) {
      fetchReplicaSetPods();
    }
  }, [replicaSet, replicaSetName, namespace, clusterName]);
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-medium">ReplicaSet Pods</h2>
          <Button
            variant="outline"
            size="sm"
            onClick={fetchReplicaSetPods}
            disabled={loading}
          >
    ...
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          Unable to fetch pods for this ReplicaSet.
        </div>
      </div>
    );
  }
  ...
  if (pods.length === 0) {
    return (
      <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
        ...
        <div className="text-center p-6 text-gray-500 dark:text-gray-400">
          No pods found for this ReplicaSet. The ReplicaSet may not have created any pods yet.
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-lg border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900/30 p-4">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-2">
        <h2 className="text-lg font-medium">ReplicaSet Pods ({filteredPods.length})</h2>
        <Button
          variant="outline"
          size="sm"
          onClick={fetchReplicaSetPods}
