...
import { ChevronRight, AlertCircle, Clock, ArrowLeft, RefreshCw, Calendar, PlayCircle, History, Check, Trash, Crosshair } from "lucide-react";
  ...
  const confirmResourceDeletion = async () => {
    ...
    try {
      ...
      await deleteResource(
        currentContext.name,
        'cronjobs',
        cronJobData.metadata?.name as string,
        {
      ...
      // Navigate back to the cronjobs list
      navigate('/dashboard/explore/cronjobs');
    } catch (err) {
      console.error('Failed to delete cronjob:', err);
      setError(err instanceof Error ? err.message : 'Failed to delete cronjob');
    } finally {
      ...
      <div className={`p-6 ${fullWidth ? 'max-w-full' : 'max-w-7xl'} mx-auto`}>
        ...
        {cronJobData && (
          <DeletionDialog
            ...
            description={`Are you sure you want to delete the cronjob "${cronJobData.metadata.name}" in namespace "${cronJobData.metadata.namespace}"? This action cannot be undone.`}
            resourceName={cronJobData.metadata.name as string}
