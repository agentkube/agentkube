...
import { ChevronRight, AlertCircle, ArrowLeft, RefreshCw, Network, ExternalLink, Server, Globe, Trash } from "lucide-react";
  ...
  const confirmResourceDeletion = async () => {
    ...
    try {
      ...
      await deleteResource(
        currentContext.name,
        'services',
        serviceData.metadata?.name as string,
        {
          namespace: serviceData.metadata?.namespace
          // Note: Services are in the core API group, so no apiGroup parameter needed
        }
      );

      // Navigate back to the services list
      navigate('/dashboard/explore/services');
    } catch (err) {
      ...
      <div className={`p-6 ${fullWidth ? 'max-w-full' : 'max-w-7xl'} mx-auto`}>
        ...
        {serviceData && (
          <DeletionDialog
            ...
            description={`Are you sure you want to delete the service "${serviceData.metadata.name}" in namespace "${serviceData.metadata.namespace}"? This action cannot be undone.`}
            resourceName={serviceData.metadata.name as string}
