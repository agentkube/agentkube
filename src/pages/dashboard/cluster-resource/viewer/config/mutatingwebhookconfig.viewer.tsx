...
const MutatingWebhookViewer: React.FC = () => {
  ...
  const fetchEvents = async () => {
    ...
    try {
      ...
      const filteredEvents = eventData.filter(event => {
        return (
          event.involvedObject?.kind === 'MutatingWebhookConfiguration' &&
  ...
  useEffect(() => {
    const fetchWebhookData = async () => {
      ...
      try {
        ...
        // Get webhook details
        const data = await getResource<'mutatingwebhookconfigurations'>(
          currentContext.name,
          'mutatingwebhookconfigurations',
          webhookName,
          undefined, // MutatingWebhookConfigurations are cluster-scoped
  ...
  const handleRefresh = () => {
    ...
    if (currentContext && webhookName) {
      Promise.all([
        getResource<'mutatingwebhookconfigurations'>(
          currentContext.name,
          'mutatingwebhookconfigurations',
          webhookName,
          undefined,
    ...
    return (
      <div className="p-6">
        <Alert>
          ...
          <AlertDescription>
            The requested MutatingWebhookConfiguration was not found or could not be retrieved.
          </AlertDescription>
        </Alert>
        <div className="mt-4">
          <Button onClick={() => navigate(-1)}>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        <Breadcrumb className="mb-6">
          <BreadcrumbList>
            ...
            <BreadcrumbItem>
              <BreadcrumbLink href="/dashboard/explore/mutatingwebhookconfigurations">Mutating Webhooks</BreadcrumbLink>
            </BreadcrumbItem>
            <BreadcrumbSeparator>
              <ChevronRight className="h-4 w-4" />
            </BreadcrumbSeparator>
                    ...
                    {webhook.reinvocationPolicy && (
                      <div className="mt-4 p-3 rounded-lg bg-yellow-50 dark:bg-yellow-900/10 border border-yellow-200 dark:border-yellow-800/50">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium">Reinvocation Policy:</span>
...
export default MutatingWebhookViewer;